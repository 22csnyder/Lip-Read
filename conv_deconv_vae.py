...
#def mnist(datasets_dir='/../../Data/kastner'):
...
class ConvVAE(PickleMixin):
    ...
    def _setup_functions(self, trX):##assumes square
    ...
    def fit(self, trX):
        ...
        t = time()
        n = 0.
        epochs = 1000 + self.epoch_
        for e in range(self.epoch_,epochs):#modified to not start at e=0 after load from snapshot
...
#                f = open(os.getcwd()+self.snapshot_file, 'wb')
    ...
    def encode(self, X, e=None):
        if e is None:
            e = floatX(np.ones((X.shape[0], self.n_code)))
        return self._z_given_x(X, e)

    def decode(self, Z):
        return self._z_given_x(Z)#bug??
...
#    trX, trY = cifar10(data_dir)
#    tf = ConvVAE(image_save_root=results_dir,snapshot_file="cifar_snapshot.pkl")
...
#    tr, _, _, = mnist(data_dir)
#    trX, trY = tr
#    tf = ConvVAE(image_save_root=results_dir,
#                 snapshot_file=results_dir+'/mnist_snapshot.pkl')




#    trX=np.load(data_dir+'generic_speaking_data_5_7_8.npy')
#    trX=trX[:22000,:,3:35,15:47]#12:50 to make it square#also gets rid of strange red borders
    
#    np.save(data_dir+'small_generic_speaking_data.npy',trX)

####active construction###
#    trX=np.load(data_dir+'generic_speaking_data_5_7_8.npy')#try without zca
#    trX=np.load(data_dir+'small_generic_speaking_data.npy')
#    trX=trX.astype(np.float)
#    trX=trX/trX.max()
#    zca=ZCA(copy=False)
#    old_shape=trX.shape    
#    X=zca.fit_transform(trX.reshape(len(trX), -1))    
#    X=X.reshape(old_shape)
#    print 'finished whitening!'



#    imshow(zca.VT[0].reshape(3,32,32))#first principle component


#    imshow(zca.VT[4].reshape(3,32,32))#first principle component

#    w=X[50]
#    _w=w.reshape(3072,-1)
    
#    _W=zca.VT.T*np.sqrt(zca.S+zca.bias)*_w
#    _W=zca.VT.T*_w
    
#    x=    
#    x=zca.VT   np.dot(VT,_w)

#    trX = zca_trX.reshape(old_shape)    
#    X=trX.reshape(len(trX),-1)
#    w_trX=zca.transform(X)


######THIS is a good section. Just happens to be commented out#######
#    trX=np.load(data_dir+'generic_speaking_data_5_7_8.npy')
...
###Tried to rescue it like this
#    from theano import config
#    config.experimental.unpickle_gpu_on_cpu=True
#    cputf = ConvVAE(image_save_root=results_dir,snapshot_file=results_dir+'/generic_speaking_5_7_8_1-22000_no-white_square_resized_snapshot.pkl')
#    config.experimental.unpickle_gpu_on_cpu=False
#    gputf = ConvVAE(image_save_root=results_dir,snapshot_file=results_dir+'/generic_speaking_5_7_8_1-22000_no-white_square_resized_snapshot.pkl')
#    maxtrX=244.
...
#    a1,b1=cputf._z_given_x(X)
#    a2,b2=gputf._z_given_x(X)
